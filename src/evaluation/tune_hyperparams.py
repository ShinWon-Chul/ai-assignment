...
async def evaluate_with_params(
    ...
    print(f"Parameters: top_k={top_k}, bm25_weight={bm25_weight:.2f}, semantic_weight={semantic_weight:.2f}")
    print(f"{'='*80}")
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"Category-wise Accuracy:\n")
...
async def run_hyperparameter_tuning(
    ...
    bm25_weight_range: tuple[float, float, float] = (0.1, 0.55, 0.05),  # (start, stop, step)
    ...
    bm25_weight_values = list(np.arange(bm25_weight_range[0], bm25_weight_range[1], bm25_weight_range[2]))
    ...
    for top_k in top_k_values:
        for bm25_weight in bm25_weight_values:
            ...
            if experiment_num < start_from:
                print(f"[{experiment_num}/{total_experiments}] Skipping (already completed)")
                continue
            ...
            try:
                ...
                with open(progress_log_path, "a", encoding="utf-8") as f:
                    ...
                    f.write(f"  top_k: {top_k}, bm25_weight: {bm25_weight:.2f}, semantic_weight: {1.0-bm25_weight:.2f}\n")
    ...
    with open(best_config_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"Accuracy: {best_config['accuracy']:.4f} ({best_config['accuracy']*100:.2f}%)\n")
    ...
    with open(progress_log_path, "a", encoding="utf-8") as f:
        ...
        f.write(f"Total Experiments Completed: {len(all_results)}/{total_experiments}\n\n")
        f.write("Top 5 Configurations by Accuracy:\n")
        f.write("-"*80 + "\n")
        for i, row in summary_df.head(5).iterrows():
            f.write(f"{i+1}. {row['experiment_id']}\n")
            f.write(f"   top_k={int(row['top_k'])}, bm25_weight={row['bm25_weight']:.2f}\n")
